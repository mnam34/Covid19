...
namespace Common.OpenXmlExcel
{
    public class ExcelDataReader : IDataReader
    {
        ...
        public ExcelDataReader(string path, int sheetIndex = 0, bool firstRowAsHeader = false, int columns = 3)
        {
            ...
            _headers = firstRowAsHeader ? GetFirstRowAsHeaders() : GetRangeHeaders(worksheetPart, columns);
        }

        public ExcelDataReader(Stream stream, int sheetIndex = 0, bool firstRowAsHeader = false, int columns = 3)
        {
        ...
        public ExcelDataReader(string path, string sheetName, bool firstRowAsHeader = false, int columns = 3)
        {
            _document = SpreadsheetDocument.Open(path, false);
            _sharedStrings = GetSharedStrings(_document);
        ...
        public ExcelDataReader(Stream stream, string sheetName, bool firstRowAsHeader = false, int columns = 3)
        {
            _document = SpreadsheetDocument.Open(stream, false);
            _sharedStrings = GetSharedStrings(_document);
        ...
        private static string[] GetRangeHeaders(OpenXmlPart worksheetPart, int columns = 3)
        {
            //Lấy cố định 3 cột đầu tiên
            var count = columns;
